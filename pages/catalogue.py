...
st.set_page_config(page_title="Catalogue - BiblioTech", page_icon="📚", layout="wide")
    ...
    with st.expander("➕ Ajouter un document", expanded=False):
        with st.form("ajout_document"):
            ...
            with col1:
                ...
                annee = st.number_input("Année", 1900, datetime.now().year, datetime.now().year)
            with col2:
            ...
            if type_document == "livre":
                ...
                with c2: specifique["editeur"] = st.text_input("Éditeur")
                ...
                with c2: specifique["universite"] = st.text_input("Université")
                with c3: specifique["annee_soutenance"] = st.number_input("Année soutenance", 1900, datetime.now().year, datetime.now().year)
                ...
                with c3: specifique["numero"] = st.text_input("Numéro")
            elif type_document == "dvd":
                c1, c2 = st.columns(2)
                with c1: specifique["realisateur"] = st.text_input("Réalisateur")
                with c2: specifique["duree"] = st.number_input("Durée (minutes)", 1, 500)
            ...
            if st.form_submit_button("💾 Enregistrer", type="primary", use_container_width=True):
                ...
                if type_document == "livre":
                    ...
                    if not specifique.get("editeur", "").strip():
                        erreurs.append("L'éditeur est obligatoire pour un livre.")
                elif type_document == "these":
                    if not specifique.get("auteur", "").strip():
                        erreurs.append("L'auteur est obligatoire pour une thèse.")
                    if not specifique.get("universite", "").strip():
                        erreurs.append("L'université est obligatoire pour une thèse.")
                if erreurs:
                    for e in erreurs:
                        st.error(e)
                else:
...
if not documents:
    ...
    for doc in documents:
        ...
        type_label = doc.get("type", "").capitalize()
        ...
        if col_actions:
            with col_actions:
                if st.button("✏️", key=f"m_{doc['_id']}", use_container_width=True):
                    st.session_state["modifier"] = str(doc["_id"])
                    st.rerun()
                with st.popover("🗑️", use_container_width=True):
                    st.warning("Supprimer ce document définitivement ?")
            ...
            with st.expander(f"⭐ Avis ({len(doc['avis'])}) — Note moyenne : {doc.get('note_moyenne', 0)}/5"):
                for avis in doc["avis"]:
                    ...
                    <div style="background:var(--gray-100);border-radius:8px;padding:0.8rem;margin-bottom:0.5rem;">
                        ...
                        <span style="float:right;color:var(--gray-600);font-size:0.85rem;">{avis['date'].strftime('%d/%m/%Y') if isinstance(avis['date'], datetime) else ''}</span><br>
...
if est_bibliothecaire and "modifier" in st.session_state:
    ...
    if doc:
        with st.expander(f"✏️ Modification : {doc['titre']}", expanded=True):
            with st.form("modifier_form"):
                col1, col2 = st.columns(2)
                with col1:
                    nouveau_titre = st.text_input("Titre", value=doc["titre"])
                    nouvelle_annee = st.number_input("Année", 1900, datetime.now().year, value=doc["annee"])
                with col2:
                ...
                if st.form_submit_button("Enregistrer", type="primary", use_container_width=True):
                    if not nouveau_titre.strip():
                        ...
                        st.error("L'année ne peut pas dépasser l'année courante.")
                    else:
