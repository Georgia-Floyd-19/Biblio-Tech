...
st.set_page_config(page_title="Tableau de bord - BiblioTech", page_icon="material:bar_chart", layout="wide")
...
st.markdown(f"<p style='color:var(--gray-500);'>Derniere mise a jour : {datetime.now().strftime('%d/%m/%Y %H:%M')}</p>", unsafe_allow_html=True)
...
k2.markdown(f"<div class='kpi-card'><div class='kpi-value' style='color:var(--success);'>{a_temps}</div><div class='kpi-label'>Retours a temps</div></div>", unsafe_allow_html=True)
...
k4.markdown(f"<div class='kpi-card'><div class='kpi-value'>{taux_conformite}%</div><div class='kpi-label'>Taux conformite</div></div>", unsafe_allow_html=True)
...
with col4:
    st.subheader("Taux de retour")
    if total_retours > 0:
        fig = go.Figure(data=[go.Pie(
            labels=["Dans les delais", "En retard"],
        ...
        st.info("Aucun retour enregistre.")
