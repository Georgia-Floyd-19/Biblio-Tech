...
st.set_page_config(page_title="Gestion emprunts - BiblioTech", page_icon="📋", layout="wide")
    ...
    for emp in encours:
        ...
        <div class="doc-card">
            ...
            <div class="doc-body">
                ...
                <div class="doc-meta">{status_badge(statut)} · Emprunté par <strong>{pseudo}</strong> · {emp['date_emprunt'].strftime('%d/%m/%Y')} → {emp['date_retour_prevue'].strftime('%d/%m/%Y')}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("✅ Retourné", key=f"ret_{emp['_id']}", use_container_width=True):
            try:
...
with tab2:
    ...
    for emp in retards:
        ...
        <div class="doc-card" style="border-color:var(--danger);">
            ...
            <div class="doc-body">
                <div class="doc-title">{emp['titre_snapshot']}</div>
                <div class="doc-meta">{status_badge('retard')} · <strong>{pseudo}</strong> · Retour prévu {emp['date_retour_prevue'].strftime('%d/%m/%Y')}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
